  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="card-details">
        ...
        <div class="detail-item"><strong>{{ roomBath }}</strong>구조</div>
        <div class="detail-item" v-if="property.floor"><strong>{{ property.floor }}층</strong>층수</div>
        <div class="detail-item" v-else><strong>{{ createdDate }}</strong>등록일</div>
      </div>
    </div>
  </div>
</template>
...
const roomBath = computed(() => {
  const r = props.property.rooms
  const b = props.property.bathrooms
...
<style scoped>
.card { background: var(--white); border-radius: 12px; border: 1px solid var(--border); overflow: hidden; cursor: pointer; transition: box-shadow 0.2s, transform 0.2s; }
.card:hover { box-shadow: 0 6px 28px rgba(27,45,79,0.1); transform: translateY(-3px); }
.card-img { height: 155px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
...
.card-name { font-size: 13px; font-weight: 600; color: var(--navy); margin-bottom: 8px; line-height: 1.45; letter-spacing: -0.2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
