<template>
  <div>
    ...
    <!-- 통계 -->
    <div class="stats-row">
      <div class="stat"><div class="stat-icon">🏠</div><div><div class="stat-num">{{ properties.length }}</div><div class="stat-label">현재 매물</div></div></div>
      ...
      <div class="stat"><div class="stat-icon">📞</div><div><div class="stat-num">031-000-0000</div><div class="stat-label">언제든 문의하세요</div></div></div>
    </div>
    ...
    <div class="listings">
      ...
      <div v-else-if="filteredProperties.length === 0" class="empty">
        등록된 매물이 없습니다.
      </div>
...
.hero h1 {
  ...
  line-height: 1.45; margin-bottom: 10px; letter-spacing: -0.5px;
}
...
.search-wrap input {
  flex: 1; font-family: 'Pretendard', sans-serif; font-size: 13px;
  border: 1.5px solid var(--border); border-radius: 8px;
  padding: 11px 16px; outline: none; background: var(--bg); transition: border-color 0.2s;
}
.search-wrap input:focus { border-color: var(--blue); background: var(--white); }
.search-wrap button {
  background: var(--blue); color: #fff; border: none; border-radius: 8px;
  padding: 11px 24px; font-family: 'Pretendard', sans-serif;
...
.deal-tabs { display: flex; gap: 6px; padding: 12px 0; align-items: center; }
.deal-label { font-size: 11px; color: var(--muted); margin-right: 4px; font-weight: 500; }
.deal-tab {
  padding: 5px 15px; border-radius: 20px; font-size: 12px; font-weight: 500;
  cursor: pointer; border: 1.5px solid var(--border); background: var(--white); color: var(--muted); transition: all 0.15s;
}
...
.listings-title { font-size: 16px; font-weight: 800; color: var(--navy); letter-spacing: -0.3px; }
